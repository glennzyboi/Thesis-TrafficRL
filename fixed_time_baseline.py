...
class FixedTimeController:
    ...
    def _apply_fixed_timing(self):
        """Apply fixed timing to all traffic lights"""
        ...
        for tl_id in self.traffic_lights:
            if tl_id in self.timing_plans:
                ...
                phases = timing_plan['phases']
                
                # Calculate position in cycle
                cycle_position = current_time % cycle_length
                
                # Simple 4-phase operation that works with actual network
                ...
                try:
                    ...
                    # Simple cycling through available phases
                    phase_duration = cycle_length / (max_phase + 1)
                    current_phase = int(cycle_position / phase_duration) % (max_phase + 1)
